...
class BackgroundManager {
  ...
  async handleSendTransaction(request, sendResponse) {
    try {
      const { transaction } = request;
      
      // Get active RPC provider
      const rpcProvider = await this.getActiveRPCProvider();
      console.log('Using RPC provider for transaction:', rpcProvider.name, rpcProvider.url);
      
      const response = await this.makeAPIRequest('/send-tx', rpcProvider, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
